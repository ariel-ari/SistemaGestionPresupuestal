<?php

use Livewire\Volt\Component;
use App\Livewire\Forms\GoalForm;
use Livewire\Attributes\On;
use Jantinnerezo\LivewireAlert\Facades\LivewireAlert;
use Illuminate\Validation\ValidationException;
use App\Traits\ResetsForm;

new class extends Component {

    use ResetsForm;

    public GoalForm $form;

    #[On('reset-form-goal')]
    public function resetFormGoal()
    {
        $this->resetForm();
    }

    public function save()
    {
        try {
            $this->form->store();
            Flux::modal('create-goals')->close();
            $this->dispatch('refresh-table.goals');
            $this->resetForm();
            LivewireAlert::title('Meta agregada')->text('La meta se agrego correctamente')->toast()->position('top-end')->success()->show();
        } catch (ValidationException $e) {
            throw $e;
        } catch (\Throwable $e) {
            LivewireAlert::title('Ocurrio un Error')->text($e->getMessage())->toast()->position('top-end')->error()->show();
        }
    }
}; ?>

<flux:modal @close="resetFormGoal" name="create-goals" class="grid gap-2 md:w-96">
    <flux:heading class="mt-2" size="lg">Agregar meta</flux:heading>
    <flux:text>Complete el codigo y el nombre de la meta.</flux:text>
    <form wire:submit='save' class="grid gap-2">
        <flux:input wire:model="form.code" label="Codigo" type="text" required placeholder="Codigo" />
        <flux:input wire:model="form.name" label="Meta" type="text" required
            placeholder="Meta" />
        <div class="flex">
            <flux:spacer />
            <flux:button type="submit" variant="primary">Agregar</flux:button>
        </div>
    </form>
</flux:modal>
